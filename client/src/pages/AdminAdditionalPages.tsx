...
export function ProjectsPage() {
  const [tab, setTab] = useState("All projects"); const [notice, setNotice] = useState<string | null>(null); const [selectedProject, setSelectedProject] = useState<(typeof projects)[number] | null>(null); const notify = (message: string) => { setNotice(message); window.setTimeout(() => setNotice(null), 2500); };
  ...
  return <div className="page-wrap section-page"><SectionHeader eyebrow="COMMUNITY  /  PASSION PROJECTS" title="Good things are taking shape." description="Help people find collaborators, encouragement, and a little momentum for the ideas they care about." action={<ActionPill primary onClick={() => notify("Project intake opened.")}><Plus size={16} /> Add project</ActionPill>} />
    ...
    <div className="project-grid">{visible.map((project, index) => <article className={cn("project-card surface-card", index === 0 && "featured-project")} key={project.title}><div className="project-photo"><img src={project.image} alt={`${project.title} project`} /><span className={cn("project-state", project.tone)}>{project.progress}</span></div><div className="project-card-body"><div className="project-label"><Sparkles size={14} /> Passion project</div><h2>{project.title}</h2><p>{project.detail}</p><div className="project-owner"><span className="mini-avatar sun">{project.owner.split(" ").map((name) => name[0]).join("")}</span><span>Started by <strong>{project.owner}</strong></span></div><div className="tag-row">{project.tags.map((tag) => <span key={tag}>{tag}</span>)}</div><div className="card-action-row"><ActionPill primary onClick={() => setSelectedProject(project)}>View project <ArrowRight size={15} /></ActionPill><button className="icon-button" type="button" onClick={() => notify("Project actions coming soon.")} aria-label="More project actions"><MoreHorizontal size={18} /></button></div></div></article>)}</div>{selectedProject ? <DetailDrawer eyebrow="PASSION PROJECT" title={selectedProject.title} description="See what this project is about, who started it, and how the community can help it move forward." onClose={() => setSelectedProject(null)} actions={<><DrawerAction onClick={() => setSelectedProject(null)}>Close details</DrawerAction><DrawerAction primary onClick={() => notify(`${selectedProject.title} shared with the community.`)}><ArrowUpRight size={15} /> Share project</DrawerAction></>}><div className="drawer-project-hero"><img src={selectedProject.image} alt={`${selectedProject.title} project`} /><span className={cn("project-state", selectedProject.tone)}>{selectedProject.progress}</span></div><DrawerMeta items={[{ label: "Started by", value: selectedProject.owner }, { label: "Project state", value: selectedProject.progress }, { label: "Community tags", value: selectedProject.tags.join(" · ") }]} /><div className="drawer-section"><h3>Project brief</h3><p className="drawer-body-copy">{selectedProject.detail}</p><div className="drawer-callout"><Sparkles size={16} /><span>Projects grow through small, generous contributions. Invite people into the next clear step.</span></div></div></DetailDrawer> : null}{notice ? <Notice message={notice} onClose={() => setNotice(null)} /> : null}</div>;
}
